...
take_instant_full() {
    ...
    mv "$HOME/$NAME" "$screenshot_folder/" 2>/dev/null
    FILE="$screenshot_folder/$NAME"

    dunstify "Screenshot saved" "Click to open" \
        -a "Screen Capture" \
        -i "$FILE" \
        -h string:image-path:"$FILE" \
        -A "open,Open" \
        -t 2500 | while read -r action; do
            if [[ "$action" == "open" ]]; then
                xdg-open "$FILE"
            fi
        done
...
take_instant_area() {
    ...
    # notif + action
    dunstify "Screenshot saved" "Click Open to preview" \
        -a "Screen Capture" \
        -i "$FILE" \
        -h string:image-path:"$FILE" \
        -A "open,Open" \
...
# Quick instant mode: area selection and copy only (no save)
take_instant_copy_area() {
    ...
    # capture to clipboard using grim and wl-copy
    grim -g "$region" - | wl-copy && notify-send \
        "Screenshot copied" \
        "Area ready to paste (Ctrl+V)" \
        -a "Screen Capture" \
        -i "edit-copy-symbolic" \
        -t 1500
}
